...
class ALSTM_Cell(Layer):
    ...
    def build(self, input_shape):
        ...
        self.attn_kernel = self.add_weight(
            ...
            regularizer=self.recurrent_regularizer,
            constraint=self.recurrent_constraint)
        self.attn_bias = self.add_weight(
            shape=(self.units,),
            name='attn_bias',
            initializer=self.attn_bias_initializer,
    ...
    def call(self, inputs, states, training=None):
        ...
        if(self.batch_normalization):
            z += BatchNormalization()(K.dot(h_tm1, self.recurrent_kernel))
        else:
...
#         print (time.shape)
        ti = time * self.attn_kernel + self.attn_bias
        
        a = self.recurrent_activation(ti)
        i = self.recurrent_activation(z0)
        f = self.recurrent_activation(z1)
        c = a * f * c_tm1 + i * self.activation(z2)
        o = self.recurrent_activation(z3)

        if(self.batch_normalization):
            h = o * self.activation(BatchNormalization()(c))
        else:
